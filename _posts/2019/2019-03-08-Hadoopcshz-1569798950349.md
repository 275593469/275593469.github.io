...
* native.lib.available=true

是否启动Hadoop的本地库，默认启用。本地库可以加快基本操作，例如IO，压缩等。

* http.filter.initializers=org.apache.hadoop.http.lib.StaticUserWebFilter
...
* seqfile.local.dir=${hadoop.tmp.dir}/io/local
...
* _fs.AbstractFileSystem.file.impl=org.apache.hadoop.fs.local.LocalFs_

文件系统实现类：file

* AbstractFileSystem.hdfs.impl=org.apache.hadoop.fs.Hdfs

文件系统实现类：hdfs

* AbstractFileSystem.viewfs.impl=org.apache.hadoop.fs.viewfs.ViewFs

文件系统实现类：viewfs (例如客户端挂载表)。

**在实现federation特性时，客户端可以部署此系统，方便同时访问多个nameservice**

* _fs.ftp.host=0.0.0.0_

非Hdfs文件系统设置。_暂不关注_

* _fs.ftp.host.port=21_
...
* _fs.s3.block.size=67108864_

非Hdfs文件系统设置。_暂不关注_

* _fs.s3.buffer.dir=${hadoop.tmp.dir}/s3_

非Hdfs文件系统设置。_暂不关注_

* _fs.s3.maxRetries=4_

非Hdfs文件系统设置。_暂不关注_

* _fs.s3.sleepTimeSeconds=10_

非Hdfs文件系统设置。_暂不关注_

* automatic.close=true

默认的，文件系统实例在程序退出时自动关闭，通过JVM shutdown hook方式。可以把此属性设置为false取消这种操作。这是一个高级选项，需要使用者特别关注关闭顺序。_不要关闭_

* _fs.s3n.block.size=67108864_

非Hdfs文件系统设置。_暂不关注_

* seqfile.compress.blocksize=1000000

SequenceFiles以块压缩方式压缩时，块大小大于此值时才启动压缩。

* seqfile.lazydecompress=TRUE

懒惰解压，仅在必要时解压，仅对块压缩的SequenceFiles有效。

* seqfile.sorter.recordlimit=1000000

在SequenceFiles.Sorter spill过程中，保存在内存中的记录数

* mapfile.bloom.size=1048576

在BloomMapFile使用的布隆过滤器内存大小。

* mapfile.bloom.error.rate=0.005

BloomMapFile中使用布隆过滤器失败比率. 如果减少这个值，使用的内存会成指数增长。

* util.hash.type=murmur

默认Hash算法实现. &#39;murmur&#39;:MurmurHash, &#39;jenkins&#39;:JenkinsHash.

* client.idlethreshold=4000
...
* client.connection.maxidletime=10000

毫秒，最大时间，超过后客户端会断开和服务器的连接。

* client.connect.max.retries=10

客户端连接重试次数。

* client.connect.max.retries.on.timeouts=45

在连接超时后，客户端连接重试次数

* server.listen.queue.size=128

定义服务器端接收客户端连接的监听队列长度

* server.tcpnodelay=false
...
* rpc.socket.factory.class.ClientProtocol=null
...
* _file.stream-buffer-size=4096_

非hdfs文件系统，暂不关注

* _s3.stream-buffer-size=4096_

非hdfs文件系统，暂不关注

* _kfs.stream-buffer-size=4096_

非hdfs文件系统，暂不关注

* _ftp.stream-buffer-size=4096_

非hdfs文件系统，暂不关注

* _tfile.io.chunk.size=1048576_

非hdfs文件系统，暂不关注

* http.authentication.type=simple

Oozie Http终端安全验证。可选值：simple \| kerberos \|#AUTHENTICATION_HANDLER\_CLASSNAME#

**建议simple，关闭验证**

* _hadoop.http.authentication.token.validity=36000_

安全选项。_暂不关注_

* _hadoop.http.authentication.signature.secret.file=${user.home}/hadoop-http-auth-signature-secret_

安全选项。_暂不关注_

* _hadoop.http.authentication.cookie.domain=null_

安全选项。_暂不关注_

* _hadoop.http.authentication.simple.anonymous.allowed=TRUE_

安全选项。_暂不关注_

* _hadoop.http.authentication.kerberos.principal=HTTP/\[email protected]_

安全选项。_暂不关注_

* _hadoop.http.authentication.kerberos.keytab=${user.home}/hadoop.keytab_

安全选项。_暂不关注_

* ha.fencing.ssh.connect-timeout=30000

SSH连接超时，毫秒，仅适用于内建的sshfence fencer。

* zookeeper.parent-znode=/hadoop-ha
...
* _hadoop.ssl.keystores.factory.class=org.apache.hadoop.security.ssl.FileBasedKeyStoresFactory_

安全选项。_暂不关注_

* _hadoop.ssl.require.client.cert=FALSE_

安全选项。_暂不关注_

* _hadoop.ssl.hostname.verifier=DEFAULT_

安全选项。_暂不关注_

* _hadoop.ssl.server.conf=ssl-server.xml_

安全选项。_暂不关注_

* _hadoop.ssl.client.conf=ssl-client.xml_

安全选项。_暂不关注_

* _hadoop.ssl.enabled=FALSE_

安全选项。_暂不关注_

* _hadoop.jetty.logs.serve.aliases=TRUE_

是否允许在Jetty中使用别名服务。

* health-monitor.connect-retry-interval.ms=1000

HA功能的健康监控连接重试间隔

* health-monitor.check-interval.ms=1000

HA功能的健康监控连接间隔

* health-monitor.sleep-after-disconnect.ms=1000

HA功能的健康监控，在因网络问题失去连接后休眠多久。_用于避免立即重试，此时网络问题仍在，没有意义_

* health-monitor.rpc-timeout.ms=45000

HA功能健康监控的超时时间，毫秒

* failover-controller.new-active.rpc-timeout.ms=60000

FC等待新的NN变成active状态的超时时间。

* failover-controller.graceful-fence.rpc-timeout.ms=5000

FC等待旧的active变成standby的超时时间。

* failover-controller.graceful-fence.connection.retries=1

FC在做完美隔离是的连接重试次数（graceful fencing）

* failover-controller.cli-check.rpc-timeout.ms=20000

手动运行的FC功能（从CLI）等待健康检查、服务状态的超时时间。

hdfs-default.xml

* hdfs.configuration.version=1
...
安全模式扩展存在时间，在需要的阀值达到后，毫秒。_可以设置为0，或者比较短的一个时间，例如3秒_
...
* client.socket-timeout=60\*1000
* datanode.socket.write.timeout=8\*60\*1000
* datanode.socket.reuse.keepalive=1000
* namenode.logging.level=info

DFS的NN的日志等级。值可以是：info，dir(跟踪命名空间变动)，&quot;block&quot; (跟踪块的创建删除，replication变动)，或者&quot;all&quot;.

* namenode.secondary.http-address=0.0.0.0:50090
...
* client.https.need-auth=FALSE

安全选项，暂不关注

* https.server.keystore.resource=ssl-server.xml

安全选项，暂不关注

* client.https.keystore.resource=ssl-client.xml

安全选项，暂不关注

* datanode.https.address=0.0.0.0:50475

安全选项，暂不关注

* namenode.https-address=0.0.0.0:50470

安全选项，暂不关注

* datanode.dns.interface=default

DN汇报它的IP地址的网卡。_我们给DN指定了0.0.0.0之类的地址，这个地址需要被解析成对外地址，这里指定的是网卡名，即那个网卡上绑定的IP是可以对外的IP，一般的，默认值就足够了_

* datanode.dns.nameserver=default

DNS的域名或者IP地址。DN用它来确定自己的域名，在对外联系和显示时调用。_一般的，默认值就足够了_

* namenode.backup.address=0.0.0.0:50100

NN的BK节点地址和端口，0表示随机选用。_使用HA，就不需要关注此选项了。建议不使用BK节点_

* namenode.backup.http-address=0.0.0.0:50105

_使用HA，就不需要关注此选项了。建议不使用BK节点_

* namenode.replication.considerLoad=true

设定在选择存放目标时是否考虑负载。_需要_

* default.chunk.view.size=32768

在浏览器中查看一个文件时，可以看到的字节数。

* namenode.name.dir=file://${hadoop.tmp.dir}/dfs/name
...
* namenode.fs-limits.max-component-length=0

路径中每个部分的最大字节长度（目录名，文件名的长度）。0表示不检查长度。_长文件名影响性能_

* namenode.fs-limits.max-directory-items=0

设置每个目录最多拥有多少个子目录或者文件。0表示无限制。_同一目录下子文件和目录多影响性能_

* namenode.fs-limits.min-block-size=1048576

最小的Block大小，字节。在NN创建时强制验证。避免用户设定过小的Block Size，导致过多的Block，这非常影响性能。

* namenode.fs-limits.max-blocks-per-file=1048576

每个文件最大的Block数。在NN写时强制检查。用于防止创建超大文件。

* block.access.token.enable=FALSE

访问DN时是否验证访问令牌。_建议false，不检查_

* block.access.key.update.interval=600

安全选项，暂不关注

* block.access.token.lifetime=600

安全选项，暂不关注

* datanode.data.dir.perm=700

本地数据目录权限设定。8进制或者符号方式都可以。

* blockreport.intervalMsec=21600000

数据块汇报间隔，毫秒，默认是6小时。

* blockreport.initialDelay=0

第一次数据块汇报时延迟，秒。_目的是减轻NN压力？_

* datanode.directoryscan.interval=21600

DN的数据块扫描间隔，秒。磁盘上数据和内存中数据调整一致。

* datanode.directoryscan.threads=1

线程池要有多少线程用来并发的压缩磁盘的汇报数据。

* namenode.safemode.min.datanodes=0

NN收到回报的DN的数量的最小值，达不到此值，NN不退出安全模式。（在系统启动时发生作用）。\&lt;=0的值表示不关心DN数量，在启动时。大于DN实际数量的值会导致无法离开安全模式。_建议不设置此值_

* namenode.max.objects=0

DFS支持的最大文件、目录、数据块数量。0无限制。

* namenode.decommission.interval=30

NN周期性检查退役是否完成的间隔，秒。

* namenode.decommission.nodes.per.interval=5

NN检查退役是否完成，每dfs.namenode.decommission.interval秒检查的节点数量。

* namenode.replication.interval=3

NN周期性计算DN的副本情况的频率，秒。

* namenode.accesstime.precision=3600000

HDFS文件的访问时间精确到此值，默认是1小时。0表示禁用访问时间。

* datanode.plugins=null

DN上的插件列表，逗号分隔。

* namenode.plugins=null
...
* client.write.exclude.nodes.cache.expiry.interval.millis=600000

最大周期去让DN保持在例外节点队列中。毫秒。操过此周期，先前被排除的DN将被移除缓存并被尝试再次申请Block。默认为10分钟。

* namenode.checkpoint.dir=file://${hadoop.tmp.dir}/dfs/namesecondary

本地文件系统中，DFS SNN应该在哪里存放临时\[用于合并\|合并后\]（to merge）的Image。如果是逗号分隔的目录列表，Image文件存放多份。冗余备份。_建议不使用SNN功能，忽略此配置_

* namenode.checkpoint.edits.dir=${dfs.namenode.checkpoint.dir}

_建议不使用SNN功能，忽略此配置_

* namenode.checkpoint.period=3600
...
* namenode.checkpoint.check.period=60

_建议不使用SNN功能，忽略此配置_

* namenode.checkpoint.max-retries=3

_建议不使用SNN功能，忽略此配置_

* namenode.num.checkpoints.retained=2

_建议不使用SNN功能，忽略此配置_

* namenode.num.extra.edits.retained=1000000

数量限制，额外的edits事务数。

* namenode.max.extra.edits.segments.retained=10000

extra edit日志文件segments的最大数量。除了用于NN重启时的最小edits文件之外。_一个segments包含多个日志文件_

* namenode.delegation.key.update-interval=86400000
...
* namenode.delegation.token.renew-interval=86400000
...
* client.use.datanode.hostname=FALSE

是否客户端应该使用DN的HostName，在连接DN时，默认是使用IP。

* datanode.use.datanode.hostname=FALSE

是否DN应该使用HostName连接其它DN，在数据传输时。默认是是IP。

* client.local.interfaces=null
...
* namenode.kerberos.internal.spnego.principal=${dfs.web.authentication.kerberos.principal}

_安全选项，暂不关注_

* secondary.namenode.kerberos.internal.spnego.principal=${dfs.web.authentication.kerberos.principal}

_安全选项，暂不关注_

* namenode.avoid.read.stale.datanode=FALSE
...
* namenode.write.stale.datanode.ratio=0.5f

当全部DN被标记为脏DN的比率高于此阀值，停止不写数据到脏DN的策略，以免造成热点问题（有效的，可写的DN太少，压力太大）。

* namenode.invalidate.work.pct.per.iteration=0.32f

高级属性。改变需小心。

* namenode.replication.work.multiplier.per.iteration=2

高级属性。改变需小心。

* webhdfs.enabled=FALSE

在NN和DN上开启WebHDFS (REST API)功能。

**可以开启尝试**

* fuse.connection.timeout=300
...
* metrics.percentiles.intervals=null
...
* **yarn.application.classpath=$HADOOP\_CONF\_DIR,$HADOOP\_COMMON\_HOME/share/hadoop/common/**** ,$HADOOP\_COMMON\_HOME/share/hadoop/common/lib/ ****,$HADOOP\_HDFS\_HOME/share/hadoop/hdfs/**** ,$HADOOP\_HDFS\_HOME/share/hadoop/hdfs/lib/ ****,$YARN\_HOME/share/hadoop/yarn/\*,$YARN\_HOME/share/hadoop/yarn/lib/\***

YARN应用的CLASSPATH，逗号分隔列表。

* app.mapreduce.am.job.task.listener.thread-count=30

MR AM处理RPC调用的线程数。

* app.mapreduce.am.job.client.port-range=null

MR AM能够绑定使用的端口范围。例如：50000-50050,50100-50200。 如果你先要全部的有用端口，可以留空（默认值null）。

* app.mapreduce.am.job.committer.cancel-timeout=60000

毫秒，如果job被kill了，等待output committer取消操作的时间。

* app.mapreduce.am.scheduler.heartbeat.interval-ms=1000

MR AM发送心跳到RM的时间间隔，毫秒

* app.mapreduce.client-am.ipc.max-retries=3

在重新连接RM获取Application状态前，客户端重试连接AM的次数。

* app.mapreduce.client.max-retries=3

客户端重连RM/HS/AM的次数。这是基于ipc接口上的规则

* ipc.client.factory.class=null

创建客户端IPC类的工厂类

* ipc.serializer.type=protocolbuffers

使用哪种序列化类

* ipc.server.factory.class=null

创建IPC服务类的工厂类

* ipc.exception.factory.class=null

创建IPC异常的工厂类

* ipc.record.factory.class=null

创建序列化记录的工厂类

* ipc.rpc.class=org.apache.hadoop.yarn.ipc.HadoopYarnProtoRPC

RPC类实现类

* resourcemanager.client.thread-count=50
...
* resourcemanager.principal=null

_安全选项_

* resourcemanager.scheduler.client.thread-count=50

调度器用于处理请求的线程数

* resourcemanager.webapp.address=0.0.0.0:8088

RM的网页接口地址：端口

* resourcemanager.resource-tracker.address=0.0.0.0:8031
...
* resourcemanager.admin.client.thread-count=1

RM管理端口处理事务的线程数

* resourcemanager.amliveliness-monitor.interval-ms=1000
...
* resourcemanager.keytab=/etc/krb5.keytab
...
* resourcemanager.nm.liveness-monitor.interval-ms=1000

RM检查NM存活的时间间隔。

* resourcemanager.resource-tracker.client.thread-count=50

处理资源跟踪调用的线程数。？

* resourcemanager.delayed.delegation-token.removal-interval-ms=30000

安全选项

* resourcemanager.application-tokens.master-key-rolling-interval-secs=86400

安全选项

* resourcemanager.container-tokens.master-key-rolling-interval-secs=86400

安全选项

* nodemanager.admin-env=MALLOC\_ARENA\_MAX=$MALLOC\_ARENA\_MAX

应该从NM传送到container的环境变量

* nodemanager.container-executor.class=org.apache.hadoop.yarn.server.nodemanager.DefaultContainerExecutor

启动containers的类。

* nodemanager.container-manager.thread-count=20

用于container管理的线程数

* nodemanager.delete.thread-count=4
