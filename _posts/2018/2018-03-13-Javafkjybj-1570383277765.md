...
同样Java8也为Map接口新增了一个forEach(BiConsumer _aciton_)默认方法，BiConsumer与Consumer相似，只是BiConsumer中是accept(T t, U u)方法。
...
|         **public**** void**listJava8Test(){                List\&lt;String\&gt; lst =**new**ArrayList\&lt;\&gt;();                lst.add(&quot;abcd&quot;);                lst.add(&quot;ab&quot;);                lst.add(&quot;abc&quot;);                lst.add(&quot;aasfgd&quot;);                lst.sort((str1, str2) -\&gt; {                       **return**str1.length() - str2.length();                });                System.**out**.println(lst);//[ab, abc, abcd, aasfgd]                lst.replaceAll(str -\&gt; String._valueOf_((str.length())));                System.**out**.println(lst);//[2, 3, 4, 6]        } |
| --- |
...
|         **public**** void**test3(){                Map\&lt;String, Object\&gt; map =**new**HashMap\&lt;String, Object\&gt;();                map.put(&quot;1&quot;, &quot;壹&quot;);                map.put(&quot;2&quot;, &quot;贰&quot;);                map.put(&quot;3&quot;, &quot;叁&quot;);                map.put(&quot;4&quot;, &quot;肆&quot;);                /\*\*                \* Map java8新增方法                \* compute(key, remappingFunction) : 该方法使用remappingFunction，根据key，value计算出新的value                \*                 如果新的value不为null，则替换原值；新值为null则删除原值；新值和原值都没null，则不变。                \* forEach(action) : action为Lambda表达式，用于遍历map。                \* computeIfAbsent(key, mappingFunction) : 如果传入的key对应的value为null，则使用mappingFunction                \*                 根据key计算出新的value，将原有的key-value替换；如果传入的key不存在，则新增key-value。                \* computeIfPresent(key, remappingFunction) : 如果传入的key对应的value不为null，则使用remappingFunction                \*                 根据key，value计算出新的value，新的value不为null，则替换原value；如果新的value为null，则删除原key-value对。                \* getOrDefault(key, defaultValue) : 根据key获取对应的value，如果key不存在则返回defaultValue。                \* map.merge(key, value, remappingFunction) : 该方法会先根据传入的参数key获取对应的value，如果获取的value为null                \*                 则直接用传入的value替换（这种情况可能会新增一对key-value）；如果获取的value不为null，则使用remappingFunction根据                \*                 原value和新value计算出新的value替换原value。                \* putIfAbsent(key, value) : 该方法会自动检测key对应的value是否为null，为null则将value替换，不为null则不变。                \* replace(key, value) : 如果传入的key存在则替换，不存在则返回null。该方法不会添加新的key-value。                \* replace(key, oldValue, newValue) : 将指定的key-oldValue替换成传入的key-newValue。如果指定的key-oldValue存在                \*                 则进行替换并返回true，否则返回false。                \* replaceAll(function) : 使用function对原来所有的key-value进行重新计算，将新的value替换成对应key的值。                \*/                map.compute(&quot;1&quot;, (key, value) -\&gt; {                       **return**&quot;一&quot;;                });                map.computeIfAbsent(&quot;5&quot;, (key) -\&gt; {                       **return**&quot;伍&quot;;                });                map.computeIfPresent(&quot;2&quot;, (key, value) -\&gt; {                       **return**&quot;二&quot;;                });                Object obj = map.getOrDefault(&quot;10&quot;, &quot;十&quot;);                System.**out**.println(obj);                map.merge(&quot;1&quot;, &quot;一&quot;, (oldValue, newValue) -\&gt; {                        System.**out**.println(oldValue);                        System.**out**.println(newValue);                       **return**&quot;124&quot;;                });                map.putIfAbsent(&quot;1&quot;, &quot;123&quot;);                map.replaceAll((key, value) -\&gt; {                       **return**key+value;                });                map.forEach((key, value) -\&gt; {                        System.**out**.println(key);                        System.**out**.println(value);                });        } |
| --- |
